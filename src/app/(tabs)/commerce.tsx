...
const distanceLabel = (distance: number | null, language: 'es' | 'en', nearby = true) => distance == null
  ? (language === 'es' ? 'Buscar en el mapa' : 'Find on map')
  : distance < 1
    ? `${Math.round(distance * 1000)} m ${nearby ? (language === 'es' ? 'de vos' : 'away') : (language === 'es' ? 'del centro regional' : 'from region center')}`
    : `${distance.toFixed(1)} km ${nearby ? (language === 'es' ? 'de vos' : 'away') : (language === 'es' ? 'del centro regional' : 'from region center')}`;
  ...
  return (
    <Pressable accessibilityLabel={`${language === 'es' ? 'Abrir detalles de' : 'Open details for'} ${service.title}`} accessibilityRole="button" className="mb-3 min-h-24 flex-row items-center rounded-card border border-ui-border bg-ui-surface p-3 active:opacity-85 dark:border-ui-dark-border dark:bg-ui-dark-surface" onPress={() => onOpen(service)}>
      ...
      <View className="ml-3 flex-1">
        ...
        <Text className="mt-1 text-xs font-bold text-ui-primary dark:text-ui-dark-primary" numberOfLines={1}>{distanceLabel(service.distance_km, language, nearby)} · {service.price_range ?? '₡'}</Text>
...
export default function CommerceScreen() {
  const { language, requireAuth, session, userLocation, locating, locationError, refreshUserLocation, isDark } = useApp();
  const gold = appTheme.colors.commerceGold;
  ...
  const selectedRegion = useMemo<CommerceRegion | undefined>(() => regions.find((region) => region.id === regionId), [regionId, regions]);
  ...
  const catalogTitle = viewMode === 'region' ? (language === 'es' ? 'Comercios en la región' : 'Businesses in the region') : isCinemaCategory ? (language === 'es' ? 'Cines cerca de vos' : 'Cinemas near you') : (language === 'es' ? 'Resultados para vos' : 'Results for you');
  ...
  const header = (
    <View>
      ...
      <View className="mx-5 mt-5 rounded-card border border-ui-border bg-ui-surface p-4 dark:border-ui-dark-border dark:bg-ui-dark-surface">
        <View className="flex-row items-center gap-3">
          <MaterialCommunityIcons name="map-marker-radius-outline" size={24} color={isDark ? appTheme.colors.dark.primary : appTheme.colors.light.primary} />
          <View className="flex-1">
            <Text className="text-sm font-bold text-ui-text dark:text-ui-dark-text">{userLocation ? (language === 'es' ? 'Cerca de tu ubicación' : 'Near your location') : (language === 'es' ? 'Elegí tu punto de partida' : 'Choose your starting point')}</Text>
            <Text className="mt-1 text-xs leading-5 text-ui-text-muted dark:text-ui-dark-text-muted">{userLocation ? (language === 'es' ? 'Distancias en línea recta. Más cercanos primero.' : 'Straight-line distances. Nearest first.') : (language === 'es' ? 'Activá tu ubicación para ordenar por cercanía real.' : 'Enable your location to sort by your actual proximity.')}</Text>
          </View>
        </View>
        <View className="mt-3"><ActionButton label={language === 'es' ? (userLocation ? 'Actualizar mi ubicación' : 'Usar mi ubicación') : (userLocation ? 'Refresh my location' : 'Use my location')} emphasis="outline" busy={locating} onPress={() => void refreshUserLocation()} /></View>
        {locationError ? <Text accessibilityRole="alert" className="mt-3 text-xs leading-5 text-ui-warning dark:text-ui-dark-warning">{locationError === 'denied' ? (language === 'es' ? 'Permití la ubicación en los ajustes del dispositivo o navegador, o elegí una región.' : 'Allow location in your device or browser settings, or choose a region.') : (language === 'es' ? 'No pudimos actualizar el GPS. Activá la ubicación e intentá de nuevo. Si ya había una ubicación, las distancias conservan ese punto.' : 'GPS could not refresh. Enable location and retry. If a location was available, distances still use that position.')}</Text> : null}
        {!userLocation ? <View className="mt-3">
          <Text className="mb-1 text-xs font-bold text-ui-text-muted dark:text-ui-dark-text-muted">{language === 'es' ? 'O explorá una región' : 'Or explore a region'}</Text>
          <Picker accessibilityLabel={language === 'es' ? 'Región de búsqueda' : 'Search region'} selectedValue={regionId ?? ''} onValueChange={(value) => setRegionId(value || undefined)} style={{ color: isDark ? appTheme.colors.dark.text : appTheme.colors.light.text }}>
            <Picker.Item label={language === 'es' ? 'Seleccionar región' : 'Select region'} value="" />
            {regions.map((region) => <Picker.Item key={region.id} label={language === 'es' ? region.name_es : region.name_en} value={region.id} />)}
          </Picker>
          {selectedRegion ? <Text className="mt-1 text-xs text-ui-text-muted dark:text-ui-dark-text-muted">{language === 'es' ? `Distancias desde el centro de ${selectedRegion.name_es}, hasta ${selectedRegion.radius_km} km.` : `Distances from ${selectedRegion.name_en} center, within ${selectedRegion.radius_km} km.`}</Text> : null}
          {regionsQuery.isError ? <ActionButton label={language === 'es' ? 'Reintentar regiones' : 'Retry regions'} emphasis="ghost" onPress={() => void regionsQuery.refetch()} /> : null}
        </View> : null}
      </View>
    ...
    <View className="flex-1 bg-[#F8F6F0] dark:bg-ui-dark-background">
      ...
      <Modal visible={registerOpen} transparent animationType="slide" onRequestClose={() => setRegisterOpen(false)}>
        <View className="flex-1 justify-end bg-black/40"><View className="max-h-[92%] rounded-t-3xl bg-ui-surface p-6 dark:bg-ui-dark-surface">
          ...
          <Text className="mt-1 text-sm text-ui-text-muted dark:text-ui-dark-text-muted">{language === 'es' ? `Completá la información. Se enviará a revisión y, cuando un administrador lo apruebe, se mostrará según cercanía desde ${viewMode === 'nearby' ? 'tu ubicación' : selectedRegion?.name_es ?? 'la región seleccionada'}.` : `Complete the information. It will be reviewed and, after an administrator approves it, shown by distance from ${viewMode === 'nearby' ? 'your location' : selectedRegion?.name_en ?? 'the selected region'}.`}</Text>
