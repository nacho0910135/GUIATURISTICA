...
function ServiceCard({ service, saved, onClaim, onOpen, onReport, onSaved, subcategoryOptions }: { service: CommerceService; saved: boolean; onClaim: (service: CommerceService) => void; onOpen: (service: CommerceService) => void; onReport: (service: CommerceService) => void; onSaved: (service: CommerceService) => void; subcategoryOptions: AppOption[] }) {
  const { language } = useApp();
      ...
      <View className="p-5">
        <View className="flex-row items-start">
          <View className="h-12 w-12 items-center justify-center rounded-2xl bg-ui-primary-soft dark:bg-ui-dark-primary-soft">
            <MaterialCommunityIcons name={isCinema ? 'movie-open-outline' : 'storefront-outline'} size={25} color="#087443" />
          </View>
        ...
        <View className="mt-4 border-t border-ui-border pt-4 dark:border-ui-dark-border">
          ...
          <View className="mt-2 flex-row flex-wrap gap-x-4">
            ...
            {service.booking_url ? <Pressable accessibilityRole="link" className="min-h-11 justify-center" onPress={() => { void recordBusinessEvent(service.id, 'reservation', attribution); void Linking.openURL(service.booking_url!); }}><Text className="text-xs font-black text-ui-primary">{isCinema ? (language === 'es' ? 'Cartelera y boletos' : 'Showtimes & tickets') : (language === 'es' ? 'Reservar' : 'Book')}</Text></Pressable> : null}
...
export default function CommerceScreen() {
  ...
  const header = (
    <View>
      <View className="border-b border-ui-border bg-ui-surface px-5 pb-3 pt-3 dark:border-ui-dark-border dark:bg-ui-dark-surface">
        ...
        <Text className="mt-1 text-3xl font-extrabold tracking-tight text-ui-text dark:text-ui-dark-text">{language === 'es' ? 'Comercios y servicios' : 'Businesses & services'}</Text>
        <Text className="mt-1 max-w-xl text-sm leading-5 text-ui-text-muted dark:text-ui-dark-text-muted">{language === 'es' ? 'Todo lo útil para tu viaje, organizado por experiencia y cercanía.' : 'Everything useful for your trip, organized by experience and proximity.'}</Text>
        <ScrollView horizontal className="mt-3" contentContainerStyle={{ gap: 10 }} showsHorizontalScrollIndicator={false}>
      ...
      <View className="pt-6"><Text className="px-5 text-xs font-black uppercase tracking-[1.5px] text-ui-text-muted dark:text-ui-dark-text-muted">{language === 'es' ? '¿Qué necesitás?' : 'What do you need?'}</Text><View className="mt-3 flex-row flex-wrap px-5">{categories.map((item) => <Pressable accessibilityRole="button" accessibilityState={{ selected: category === item.id }} key={item.id} onPress={() => { setCategory(item.id); setSubcategory(undefined); }} style={{ width: '25%' }} className={category === item.id ? 'items-center rounded-card border border-ui-primary bg-ui-primary-soft px-2 py-1 dark:bg-ui-dark-primary-soft' : 'items-center rounded-card border border-ui-border bg-ui-surface px-2 py-1 dark:border-ui-dark-border dark:bg-ui-dark-surface'}><View className={category === item.id ? 'h-9 w-9 items-center justify-center rounded-2xl bg-ui-primary dark:bg-ui-dark-primary' : 'h-9 w-9 items-center justify-center rounded-2xl bg-ui-muted dark:bg-ui-dark-muted'}><MaterialCommunityIcons name={(item.icon ?? 'store-outline') as React.ComponentProps<typeof MaterialCommunityIcons>['name']} size={19} color={category === item.id ? 'white' : '#087443'} /></View><Text className={category === item.id ? 'mt-1 text-center text-xs font-black text-ui-primary dark:text-ui-dark-primary' : 'mt-1 text-center text-xs font-bold text-ui-text dark:text-ui-dark-text'} numberOfLines={2}>{language === 'es' ? item.label_es : item.label_en}</Text></Pressable>)}</View></View>
