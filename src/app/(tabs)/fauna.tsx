        ...
        <View className="mt-3 flex-row flex-wrap gap-3">{filteredSpecies.map((item) => {
          ...
          >
            ...
            <View className="ml-2 gap-2">
              ...
              >
                {marking ? <FrogLoader color={seen ? 'white' : colors.primary} size="small" /> : <MaterialCommunityIcons name={seen ? 'check' : 'plus'} size={22} color={seen ? 'white' : colors.primary} />}
              </Pressable>
...
function FaunaProposalModal({ language, onClose: close, onPublished, open, userId }: { language: 'es' | 'en'; onClose: () => void; onPublished: (name: string) => void; open: boolean; userId?: string }) {
  ...
  const submit = async () => {
    ...
    try {
      setTranslating(true);
      const descriptionEn = await translateDescriptionToEnglish(description);
      setTranslating(false);
      await addFaunaSpecies({ commonName, scientificName, category, description, descriptionEn, habitat, province, userId, image });
    ...
    } finally { setSending(false); setTranslating(false); }
  };
