...
export default function ProfileScreen() {
  ...
  const tabs: { key: Section; icon: React.ComponentProps<typeof MaterialCommunityIcons>['name']; label: string; count?: number }[] = [
    ...
    { key: 'messages', icon: 'message-outline', label: tr(language, 'Mensajes', 'Messages'), count: data?.messages.filter((m) => !m.read_status).length ?? 0 },
  ...
  return <ScrollView className="flex-1 bg-ui-background dark:bg-ui-dark-background" contentContainerStyle={{ padding: 20, paddingBottom: 60 }}>
    ...
    <View className="mx-auto mt-5 w-full max-w-5xl rounded-[30px] border border-ui-border dark:border-ui-dark-border bg-ui-surface dark:bg-ui-dark-surface p-5">
      ...
      {data && section === 'notifications' ? <ListEmpty empty={!data.notifications.length} language={language}>{data.notifications.map((item) => <Row key={item.id} icon="bell-outline" title={`${item.actor?.full_name || item.actor?.username || tr(language, 'Un viajero', 'A traveler')} ${notificationText(item.type, language)}`} date={item.created_at} language={language} />)}</ListEmpty> : null}
      ...
      {data && section === 'messages' ? <ListEmpty empty={!data.messages.length} language={language}>{data.messages.map((item) => <Row key={item.id} icon="message-text-outline" title={`${item.sender?.full_name || item.sender?.username || tr(language, 'Viajero', 'Traveler')}: ${item.body}`} date={item.created_at} language={language} />)}</ListEmpty> : null}
...
function AdminPanel({ data, busy, language, refresh, run, signOut }: { data?: AdminDashboard; busy: boolean; language: 'es' | 'en'; refresh: () => Promise<void>; run: (action: () => Promise<void>) => Promise<void>; signOut: () => Promise<void> }) {
  ...
  return <View><View className="flex-row items-center justify-between"><Title>{tr(language, 'Panel del administrador', 'Admin dashboard')}</Title><Button label={tr(language, 'Salir', 'Sign out')} outline onPress={() => void signOut()} /></View>
    ...
    <Text className="mb-3 mt-6 text-lg font-bold text-ui-text dark:text-ui-dark-text">{tr(language, 'Sugerencias recibidas', 'Received suggestions')}</Text><ListEmpty empty={!data.suggestions.length} language={language}>{data.suggestions.map((item) => <Row key={item.id} icon="lightbulb-outline" title={`${profileName(item.user)}: ${item.message}`} date={item.created_at} language={language} />)}</ListEmpty>
    <Text className="mb-3 mt-6 text-lg font-bold text-ui-text dark:text-ui-dark-text">{tr(language, 'Reportes de información', 'Information reports')}</Text><ListEmpty empty={!data.reports.length} language={language}>{data.reports.map((report) => <View className="mb-3 rounded-2xl bg-ui-muted p-4 dark:bg-ui-dark-muted" key={report.id}><Text className="font-black text-ui-text dark:text-ui-dark-text">{report.target_label}</Text><Text className="mt-1 text-xs font-bold text-ui-text-muted dark:text-ui-dark-text-muted">{reportTypeLabel(report.report_type, language)} · {report.status}</Text>{report.details ? <Text className="mt-2 text-sm text-ui-text-muted dark:text-ui-dark-text-muted">{report.details}</Text> : null}<View className="mt-3 flex-row flex-wrap gap-2"><Button label={tr(language, 'Tomar revisión', 'Review')} disabled={busy} onPress={() => void run(async () => { await updateInformationReportStatus(report.id, 'reviewing'); await refresh(); })} /><Button label={tr(language, 'Resolver', 'Resolve')} disabled={busy} onPress={() => void run(async () => { await updateInformationReportStatus(report.id, 'resolved'); await refresh(); })} /><Button label={tr(language, 'Descartar', 'Dismiss')} outline disabled={busy} onPress={() => void run(async () => { await updateInformationReportStatus(report.id, 'dismissed'); await refresh(); })} /></View></View>)}</ListEmpty>
