...
function WeatherDestinationAlert({ destination, language, onPress }: { destination: Destination; language: 'es' | 'en'; onPress: () => void }) {
  ...
  return <Pressable accessibilityLabel={`${language === 'es' ? 'Abrir alerta de' : 'Open alert for'} ${destination.name}`} accessibilityRole="button" className={`w-full rounded-[18px] border p-3 active:opacity-80 ${alert.level === 'warning' ? 'border-coral-500 bg-coral-50 dark:bg-coral-500/15' : 'border-ui-border bg-ui-surface dark:border-ui-dark-border dark:bg-ui-dark-surface'}`} onPress={onPress}><View className="flex-row items-center"><MaterialCommunityIcons name={alert.level === 'warning' ? 'alert-circle' : 'weather-partly-cloudy'} size={20} color={alert.level === 'warning' ? '#ff5d52' : '#0077A8'} /><View className="ml-2.5 flex-1"><Text className="text-sm font-black text-ui-text dark:text-ui-dark-text" numberOfLines={2}>{destination.name}</Text><Text className={`mt-0.5 text-xs font-black ${alert.level === 'warning' ? 'text-coral-600' : 'text-ui-secondary dark:text-ui-dark-secondary'}`}>{alert.title}</Text><Text className="mt-0.5 text-xs leading-4 text-ui-text-muted dark:text-ui-dark-text-muted">{alert.detail}</Text></View><MaterialCommunityIcons name="chevron-right" size={20} color="#68737A" /></View></Pressable>;
}
