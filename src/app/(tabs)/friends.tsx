...
export default function FriendsScreen() {
  ...
  const writeLocked = Boolean(session && !paidAccess.hasPaidAccess);
  ...
  const matchingPlaces = useMemo(() => normalizedPlaceSearch
    ? (explorePlaces.data ?? []).filter((place) => `${place.name} ${place.province} ${place.category}`.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().includes(normalizedPlaceSearch)).slice(0, 8)
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      const commentOffset = params.commentId ? (replySectionOffsets.current[params.postId!] ?? 0) + (commentOffsets.current[params.commentId] ?? 0) : 0;
      scrollRef.current?.scrollTo({ animated: true, y: Math.max(0, postOffset + commentOffset - 12) });
    }, 200);
  ...
  const chooseCurrentLocation = async () => {
    ...
    try {
      ...
      setLocationLabel(language === 'es' ? 'Ubicación actual' : 'Current location');
      setPublishError(undefined);
      void haptic('success');
    } catch (reason) {
  ...
  const publish = async () => {
    if (!requireAuth(language === 'es' ? 'Crear una publicación' : 'Create a post') || !paidAccess.hasPaidAccess || !session || (!body.trim() && !assets.length && !location && !recommendation)) return;
    setBusy(true);
    setPublishError(undefined);
  ...
  const respond = async (postId: string) => {
    if (!requireAuth(language === 'es' ? 'Responder una publicación' : 'Reply to a post') || !paidAccess.hasPaidAccess || !session || !reply.trim()) return;
    setBusy(true);
  ...
  const react = async (postId: string, reaction: ReactionType) => {
    if (!requireAuth(language === 'es' ? 'Dar me gusta' : 'Like a post') || !paidAccess.hasPaidAccess || !session || !wall) return;
    try {
  ...
  const reactToReply = async (replyId: string, reaction: ReactionType) => {
    if (!requireAuth(language === 'es' ? 'Reaccionar a un comentario' : 'React to a comment') || !paidAccess.hasPaidAccess || !session || !wall) return;
    try {
      ...
      void haptic('error');
      Alert.alert('Comunidad Viajera', reason instanceof Error ? reason.message : (language === 'es' ? 'No se pudo actualizar la reacción.' : 'The reaction could not be updated.'));
    }
  };
    ...
    <ScrollView ref={scrollRef} className="flex-1 bg-ui-background dark:bg-ui-dark-background" contentContainerStyle={{ alignItems: 'center', paddingBottom: 48 }} showsVerticalScrollIndicator={false}>
      ...
      <View className="w-full max-w-3xl px-4 pt-5">
        ...
        {!writeLocked ? <View className="rounded-card border border-ui-border bg-ui-surface p-4 shadow-card dark:border-ui-dark-border dark:bg-ui-dark-surface" style={communityDepth}>
          ...
          {location ? <View className="mt-3 flex-row items-center rounded-control bg-ui-primary-soft p-3 dark:bg-ui-dark-primary-soft"><MaterialCommunityIcons name="map-marker" size={24} color="#0B6B4F" /><View className="ml-2 flex-1"><Text className="font-bold text-ui-text dark:text-ui-dark-text">{locationLabel ?? (language === 'es' ? 'Ubicación lista para compartir' : 'Location ready to share')}</Text><Text className="mt-0.5 text-xs text-ui-text-muted dark:text-ui-dark-text-muted">{location.latitude.toFixed(5)}, {location.longitude.toFixed(5)}</Text></View><Pressable accessibilityLabel={language === 'es' ? 'Quitar ubicación' : 'Remove location'} accessibilityRole="button" className="h-11 w-11 items-center justify-center" onPress={() => { setLocation(undefined); setLocationLabel(undefined); }}><MaterialCommunityIcons name="close" size={20} color="#68737A" /></Pressable></View> : null}
          {locationOptionsOpen ? <View className="mt-3 rounded-2xl border border-ui-border bg-ui-muted p-3 dark:border-ui-dark-border dark:bg-ui-dark-muted"><Text className="font-black text-ui-text dark:text-ui-dark-text">{language === 'es' ? '¿Cómo querés agregar la ubicación?' : 'How do you want to add the location?'}</Text><Pressable accessibilityRole="button" className="mt-3 min-h-12 flex-row items-center justify-center rounded-control bg-ui-primary px-4 disabled:opacity-50 dark:bg-ui-dark-primary" disabled={busy} onPress={() => void chooseCurrentLocation()}><MaterialCommunityIcons name="crosshairs-gps" size={21} color="white" /><Text className="ml-2 font-black text-white">{busy ? (language === 'es' ? 'Obteniendo ubicación…' : 'Getting location…') : (language === 'es' ? 'Usar ubicación actual' : 'Use current location')}</Text></Pressable><Pressable accessibilityRole="button" className="mt-3 min-h-12 flex-row items-center justify-center rounded-control bg-ui-secondary px-4 dark:bg-ui-dark-secondary" onPress={() => setLocationPickerOpen(true)}><MaterialCommunityIcons name="map-search-outline" size={21} color="white" /><Text className="ml-2 font-black text-white">{language === 'es' ? (location ? 'Cambiar ubicación en el mapa' : 'Ubicación manual en el mapa') : (location ? 'Change map location' : 'Pick manually on map')}</Text></Pressable>{location ? <Pressable accessibilityRole="link" className="mt-3 min-h-12 flex-row items-center justify-center rounded-control border border-ui-primary px-4 dark:border-ui-dark-primary" onPress={() => void Linking.openURL(`https://www.google.com/maps/search/?api=1&query=${location.latitude},${location.longitude}`)}><MaterialCommunityIcons name="map-marker-radius" size={21} color="#0B6B4F" /><Text className="ml-2 font-black text-ui-primary dark:text-ui-dark-primary">{language === 'es' ? 'Previsualizar ubicación' : 'Preview location'}</Text><MaterialCommunityIcons name="open-in-new" size={18} color="#0B6B4F" /></Pressable> : null}</View> : null}
          <LocationPickerModal initialLocation={location} language={language} onClose={() => setLocationPickerOpen(false)} onConfirm={(coordinate, label) => { setLocation(coordinate); setLocationLabel(label); setPublishError(undefined); }} open={locationPickerOpen} title={language === 'es' ? 'Ubicación de la publicación' : 'Post location'} />
          ...
          {!recommendation ? <Pressable accessibilityRole="button" accessibilityState={{ expanded: placeSearchOpen }} className="mt-3 min-h-12 flex-row items-center justify-center rounded-control border border-ui-primary bg-ui-primary-soft px-4 dark:bg-ui-dark-primary-soft" onPress={() => { setPlaceSearchOpen((open) => !open); setPlaceSearch(''); }}><MaterialCommunityIcons name="map-search-outline" size={24} color="#0B6B4F" /><Text className="ml-2 font-black text-ui-primary dark:text-ui-dark-primary">{language === 'es' ? 'Compartir un sitio turístico' : 'Share a tourist place'}</Text></Pressable> : null}
          ...
          <View className="mt-3 flex-row items-center justify-between border-t border-ui-border dark:border-ui-dark-border pt-3">
            <View className="flex-1 flex-row gap-2"><Pressable accessibilityRole="button" className="flex-1 flex-row items-center justify-center rounded-xl bg-ui-muted px-3 py-2 shadow-card dark:bg-ui-dark-muted" style={communityControlDepth} onPress={() => void choosePhoto()}><MaterialCommunityIcons name="image-multiple" size={25} color="#0B6B4F" /><Text className="ml-2 font-bold text-ui-text dark:text-ui-dark-text">{language === 'es' ? 'Foto' : 'Photo'}</Text></Pressable><Pressable accessibilityRole="button" accessibilityState={{ expanded: locationOptionsOpen }} className="flex-1 flex-row items-center justify-center rounded-xl bg-ui-muted px-3 py-2 shadow-card dark:bg-ui-dark-muted" style={{ ...communityControlDepth, shadowColor: '#C33B3B' }} onPress={() => { if (requireAuth(language === 'es' ? 'Compartir tu ubicación' : 'Share your location')) setLocationOptionsOpen((open) => !open); }}><MaterialCommunityIcons name="map-marker-outline" size={25} color="#C33B3B" /><Text className="ml-1 font-bold text-ui-text dark:text-ui-dark-text">{language === 'es' ? 'Ubicación' : 'Location'}</Text></Pressable></View>
            <MotionPressable accessibilityRole="button" className="ml-2 min-w-24 items-center rounded-control bg-ui-primary px-4 py-3 shadow-card disabled:opacity-40 dark:bg-ui-dark-primary" disabled={busy || Boolean(session && !body.trim() && !assets.length && !location && !recommendation)} onPress={() => void publish()} style={communityDepth}>{busy ? <FrogLoader color="white" /> : <Text className="font-black text-white">{language === 'es' ? 'Publicar' : 'Post'}</Text>}</MotionPressable>
          </View>
        ...
        {wall?.posts.map((post) => {
          ...
          return <View className="mt-5 overflow-hidden rounded-card border border-ui-border bg-ui-surface shadow-card dark:border-ui-dark-border dark:bg-ui-dark-surface" key={post.id} onLayout={(event) => { postOffsets.current[post.id] = event.nativeEvent.layout.y; }} style={communityDepth}>
            <View className="p-5">
              <View className="flex-row items-center">
                ...
                {!own ? <Pressable accessibilityLabel={wall.followedUserIds.has(post.user_id) ? (language === 'es' ? 'Dejar de seguir' : 'Unfollow') : (language === 'es' ? 'Seguir' : 'Follow')} accessibilityRole="button" accessibilityState={{ busy: followBusyId === post.user_id, disabled: followBusyId === post.user_id, selected: wall.followedUserIds.has(post.user_id) }} className={wall.followedUserIds.has(post.user_id) ? 'ml-2 flex-row items-center rounded-full bg-ui-primary px-3 py-2 shadow-card disabled:opacity-50 dark:bg-ui-dark-primary' : 'ml-2 rounded-full bg-ui-primary-soft px-3 py-2 shadow-card disabled:opacity-50 dark:bg-ui-dark-primary-soft'} disabled={followBusyId === post.user_id} style={communityControlDepth} onPress={() => void follow(post.user_id)}>{followBusyId === post.user_id ? <FrogLoader color={wall.followedUserIds.has(post.user_id) ? 'white' : '#0B6B4F'} size="small" /> : <>{wall.followedUserIds.has(post.user_id) ? <MaterialCommunityIcons name="account-check" size={16} color="white" /> : null}<Text className={wall.followedUserIds.has(post.user_id) ? 'ml-1 font-black text-white' : 'font-black text-ui-primary dark:text-ui-dark-primary'}>{wall.followedUserIds.has(post.user_id) ? (language === 'es' ? 'Siguiendo' : 'Following') : (language === 'es' ? 'Seguir' : 'Follow')}</Text></>}</Pressable> : null}
              </View>
            ...
            {post.latitude != null && post.longitude != null ? <Pressable accessibilityRole="link" className="mx-5 mb-4 flex-row items-center rounded-control bg-ui-muted p-4 shadow-card dark:bg-ui-dark-muted" style={communityControlDepth} onPress={() => void openNavigation(post.latitude!, post.longitude!)}><MaterialCommunityIcons name="waze" size={25} color="#0B6B4F" /><View className="ml-3 flex-1"><Text className="font-black text-ui-text dark:text-ui-dark-text">{language === 'es' ? 'Cómo llegar con Waze' : 'Directions with Waze'}</Text><Text className="mt-1 text-xs text-ui-text-muted dark:text-ui-dark-text-muted">{post.latitude.toFixed(5)}, {post.longitude.toFixed(5)}</Text></View><MaterialCommunityIcons name="open-in-new" size={20} color="#0B6B4F" /></Pressable> : null}
            <View className="border-t border-ui-border px-5 py-3 dark:border-ui-dark-border">
              ...
              <View className="relative flex-1">
                ...
                <MotionPressable
                  ...
                  onLongPress={() => { if (!requireAuth(language === 'es' ? 'Reaccionar a una publicación' : 'React to a post')) return; void haptic('impact'); longPressedPostId.current = post.id; setReactionPickerPostId(post.id); }}
              ...
              <Pressable className="mx-1 flex-1 flex-row items-center justify-center rounded-xl bg-ui-muted py-2 shadow-card disabled:opacity-40 dark:bg-ui-dark-muted" disabled={writeLocked} style={communityControlDepth} onPress={() => { if (!requireAuth(language === 'es' ? 'Comentar una publicación' : 'Comment on a post')) return; setReplying(replying === post.id ? undefined : post.id); setReply(''); setParentReplyId(undefined); }}><MaterialCommunityIcons name="comment-outline" size={21} color="#68737A" /><Text className="ml-2 font-bold text-ui-text-muted dark:text-ui-dark-text-muted">{language === 'es' ? 'Comentar' : 'Comment'}</Text></Pressable>
            ...
            {replying === post.id ? <View className="border-t border-ui-border bg-ui-background p-4 dark:border-ui-dark-border dark:bg-ui-dark-background" onLayout={(event) => { replySectionOffsets.current[post.id] = event.nativeEvent.layout.y; }}>{postReplies.map((item) => {
              const commenterName = item.user?.username || item.user?.full_name || `Viajero ${item.user_id.slice(0, 5)}`;
              return <View className={item.parent_reply_id ? 'mb-3 ml-7 flex-row items-start' : 'mb-3 flex-row items-start'} key={item.id} onLayout={(event) => { commentOffsets.current[item.id] = event.nativeEvent.layout.y; }}>
                <Pressable accessibilityLabel={language === 'es' ? `Ver perfil de ${commenterName}` : `View ${commenterName}'s profile`} accessibilityRole="link" className="mr-3" onPress={() => router.push({ pathname: '/(aux)/traveler-profile', params: { id: item.user_id } })}>
                  {item.user?.avatar_url ? <Image accessibilityLabel={commenterName} cachePolicy="none" contentFit="cover" source={{ uri: item.user.avatar_url }} style={{ borderRadius: 19, height: 38, width: 38 }} /> : <View className="h-[38px] w-[38px] items-center justify-center rounded-full bg-ui-primary-soft dark:bg-ui-dark-primary-soft"><MaterialCommunityIcons name="account" size={21} color="#0B6B4F" /></View>}
                </Pressable>
                <View className={item.id === params.commentId ? 'flex-1 rounded-control border-2 border-ui-secondary bg-ui-primary-soft p-3 shadow-card dark:bg-ui-dark-primary-soft' : item.parent_reply_id ? 'flex-1 rounded-control border-l-4 border-ui-primary bg-ui-surface p-3 shadow-card dark:border-ui-dark-primary dark:bg-ui-dark-surface' : 'flex-1 rounded-control bg-ui-surface p-3 shadow-card dark:bg-ui-dark-surface'} style={communityControlDepth}>
                  <Pressable accessibilityRole="link" className="self-start" onPress={() => router.push({ pathname: '/(aux)/traveler-profile', params: { id: item.user_id } })}><View className="flex-row items-center"><Text className="text-xs font-black text-ui-primary dark:text-ui-dark-primary">{commenterName}</Text>{item.user?.role === 'admin' ? <AdminBadge /> : null}</View></Pressable>
                  <Text className="mt-1 text-ui-text dark:text-ui-dark-text">{item.body}</Text>
