...
export default function ClaimBusinessScreen() {
  ...
  return (
    <ScrollView className="flex-1 bg-ui-background px-5 pt-12 dark:bg-ui-dark-background" contentContainerStyle={{ paddingBottom: 44 }}>
      <Pressable accessibilityRole="button" className="mb-6 flex-row items-center self-start" onPress={() => router.back()}>
        <MaterialCommunityIcons name="arrow-left" size={22} color="#087443" />
        <Text className="ml-2 font-black text-ui-primary">{isSpanish ? 'Volver' : 'Back'}</Text>
      </Pressable>
      <View className="rounded-card border border-ui-border bg-ui-surface p-6 dark:border-ui-dark-border dark:bg-ui-dark-surface">
        ...
        {!business.isLoading && !unavailable && submitted ? (
          <View className="mt-5 rounded-2xl bg-ui-primary-soft p-4 dark:bg-ui-dark-primary-soft">
            <Text className="font-black text-ui-primary dark:text-ui-dark-primary">{isSpanish ? 'Solicitud enviada' : 'Claim sent'}</Text>
            <Text className="mt-1 leading-5 text-ui-text-muted dark:text-ui-dark-text-muted">{isSpanish ? 'Quedó pendiente de revisión. Al aprobarse, el perfil pasará a tu panel de propietario.' : 'It is pending review. Once approved, the profile will appear in your owner dashboard.'}</Text>
          </View>
        ) : null}
        {!business.isLoading && !unavailable && !submitted && business.data ? (
          <>
            <Text className="mt-2 text-lg font-black text-ui-text dark:text-ui-dark-text">{business.data.title}</Text>
            {showBusinessPlanNotice ? (
              ...
              <>
                <Text className="mt-2 leading-6 text-ui-text-muted dark:text-ui-dark-text-muted">{isSpanish ? 'Contanos cómo podemos verificar que sos la persona responsable. La solicitud queda pendiente hasta revisión administrativa.' : 'Tell us how we can verify that you are responsible for this business. The claim stays pending until administrative review.'}</Text>
                <TextInput accessibilityLabel={isSpanish ? 'Información para verificar propiedad' : 'Ownership verification information'} className="mt-5 min-h-28 rounded-2xl border border-ui-border px-4 py-3 text-ui-text dark:border-ui-dark-border dark:text-ui-dark-text" maxLength={1000} multiline onChangeText={setMessage} placeholder={isSpanish ? 'Ej.: soy representante legal; adjuntaré la evidencia solicitada.' : 'Example: I am the legal representative and can provide the requested evidence.'} textAlignVertical="top" value={message} />
                {error ? <Text accessibilityRole="alert" className="mt-3 text-sm font-semibold text-red-600">{error}</Text> : null}
                <Pressable accessibilityRole="button" className="mt-5 min-h-12 items-center justify-center rounded-2xl bg-ui-primary px-4" onPress={() => void submit()}><Text className="font-black text-white">{isSpanish ? 'Enviar solicitud' : 'Send claim'}</Text></Pressable>
              </>
            )}
          </>
        ) : null}
        {!business.isLoading && !unavailable && !submitted && !business.data ? <Text className="mt-4 text-red-600">{isSpanish ? 'No pudimos cargar este perfil.' : 'This profile could not be loaded.'}</Text> : null}
      </View>
    </ScrollView>
  );
}
