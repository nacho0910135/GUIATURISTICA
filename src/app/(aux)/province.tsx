        ...
        renderItem={({ item }) => (
          <Pressable accessibilityLabel={`${language === 'es' ? 'Abrir' : 'Open'} ${item.name}`} accessibilityRole="button" className="overflow-hidden rounded-[30px] border border-ui-border bg-ui-surface active:opacity-90 dark:border-ui-dark-border dark:bg-ui-dark-surface" onPress={() => setSelected(item)}>
            <View className="relative">
              <DestinationCarousel height={240} place={item} />
...
function DestinationModal({ language, onClose, onLike, place }: { language: 'es' | 'en'; onClose: () => void; onLike: (place: MapPlace) => Promise<void>; place?: MapPlace }) {
  ...
  const [freshnessBusy, setFreshnessBusy] = useState<DestinationFreshnessCheck>();
  ...
  const freshness = useQuery({ queryKey: ['destination-freshness', place?.id], queryFn: () => getDestinationFreshness(place!.id), enabled: Boolean(place) });
  const myFreshness = useQuery({ queryKey: ['my-destination-freshness', place?.id, session?.user.id], queryFn: () => getMyDestinationFreshness(place!.id, session!.user.id), enabled: Boolean(place && session) });
  ...
  const voteFreshness = async (check: DestinationFreshnessCheck, confirmed: boolean) => {
    if (!requireAuth(language === 'es' ? 'Confirmar información del destino' : 'Confirm destination information') || !session || freshnessBusy) return;
    setFreshnessBusy(check);
    try {
      await setDestinationFreshnessVote(place.id, session.user.id, check, confirmed);
      await Promise.all([freshness.refetch(), myFreshness.refetch()]);
    ...
    finally { setFreshnessBusy(undefined); }
  };
        ...
        <View className="max-h-full w-full max-w-5xl overflow-hidden rounded-[30px] bg-ui-surface dark:bg-ui-dark-surface">
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View className="gap-6 p-5 md:p-8">
              ...
              <CommunityFreshness freshness={freshness.data} language={language} mine={myFreshness.data} busy={freshnessBusy} onVote={voteFreshness} />
...
function CommunityFreshness({ busy, freshness, language, mine, onVote }: { busy?: DestinationFreshnessCheck; freshness?: DestinationFreshness; language: 'es' | 'en'; mine?: Partial<Record<DestinationFreshnessCheck, boolean>>; onVote: (check: DestinationFreshnessCheck, confirmed: boolean) => void }) {
  const checks: { id: DestinationFreshnessCheck; icon: keyof typeof MaterialCommunityIcons.glyphMap; label: string }[] = language === 'es'
    ? [{ id: 'open', icon: 'store-check-outline', label: '¿Sigue abierto?' }, { id: 'price', icon: 'cash-check', label: '¿Tarifa correcta?' }, { id: 'cards', icon: 'credit-card-check-outline', label: '¿Aceptan tarjeta?' }]
    : [{ id: 'open', icon: 'store-check-outline', label: 'Still open?' }, { id: 'price', icon: 'cash-check', label: 'Price correct?' }, { id: 'cards', icon: 'credit-card-check-outline', label: 'Cards accepted?' }];
  return <View className="rounded-3xl border border-ui-primary/30 bg-ui-primary-soft p-5 dark:bg-ui-dark-primary-soft"><Text className="text-lg font-black text-ui-text dark:text-ui-dark-text">{language === 'es' ? 'Confirmado por viajeros' : 'Confirmed by travellers'}</Text><Text className="mt-1 text-sm text-ui-text-muted dark:text-ui-dark-text-muted">{language === 'es' ? 'Ayudá a mantener esta ficha al día.' : 'Help keep this listing up to date.'}</Text><View className="mt-4 gap-3">{checks.map((check) => { const counts = freshness?.[check.id] ?? { confirmed: 0, notConfirmed: 0 }; return <View className="rounded-2xl bg-ui-surface p-4 dark:bg-ui-dark-surface" key={check.id}><View className="flex-row items-center"><MaterialCommunityIcons name={check.icon} size={20} color="#087443" /><Text className="ml-2 flex-1 font-black text-ui-text dark:text-ui-dark-text">{check.label}</Text><Text className="text-xs font-bold text-ui-text-muted dark:text-ui-dark-text-muted">{counts.confirmed + counts.notConfirmed} {language === 'es' ? 'votos' : 'votes'}</Text></View><View className="mt-3 flex-row gap-2"><Pressable accessibilityRole="radio" accessibilityState={{ selected: mine?.[check.id] === true, disabled: Boolean(busy) }} className={mine?.[check.id] === true ? 'flex-1 rounded-xl bg-ui-primary px-3 py-2' : 'flex-1 rounded-xl border border-ui-border px-3 py-2 dark:border-ui-dark-border'} disabled={Boolean(busy)} onPress={() => onVote(check.id, true)}><Text className={mine?.[check.id] === true ? 'text-center text-xs font-black text-white' : 'text-center text-xs font-black text-ui-primary'}>{language === 'es' ? `Sí · ${counts.confirmed}` : `Yes · ${counts.confirmed}`}</Text></Pressable><Pressable accessibilityRole="radio" accessibilityState={{ selected: mine?.[check.id] === false, disabled: Boolean(busy) }} className={mine?.[check.id] === false ? 'flex-1 rounded-xl bg-coral-600 px-3 py-2' : 'flex-1 rounded-xl border border-ui-border px-3 py-2 dark:border-ui-dark-border'} disabled={Boolean(busy)} onPress={() => onVote(check.id, false)}><Text className={mine?.[check.id] === false ? 'text-center text-xs font-black text-white' : 'text-center text-xs font-black text-coral-600'}>{language === 'es' ? `No · ${counts.notConfirmed}` : `No · ${counts.notConfirmed}`}</Text></Pressable></View></View>; })}</View></View>;
}
