...
export default function ProvinceCatalogScreen() {
  ...
  return (
    <View className="flex-1 bg-[#171615]">
      <View className="bg-[#201f1d] px-5 pb-6 pt-12">
      ...
      <View className="border-b border-white/10 bg-[#201f1d] px-5 pb-5"><View className="mx-auto w-full max-w-5xl flex-row gap-3"><Pressable accessibilityRole="button" className={sortBy === 'rating' ? 'flex-1 flex-row items-center justify-center rounded-2xl bg-[#00b981] px-4 py-3' : 'flex-1 flex-row items-center justify-center rounded-2xl border border-white/15 px-4 py-3'} onPress={() => setSortBy('rating')}><MaterialCommunityIcons name="star" size={20} color={sortBy === 'rating' ? 'white' : '#ffac16'} /><Text className="ml-2 font-black text-white">{language === 'es' ? 'Mejor calificados' : 'Top rated'}</Text></Pressable><Pressable accessibilityRole="button" className={sortBy === 'nearby' ? 'flex-1 flex-row items-center justify-center rounded-2xl bg-[#13aee8] px-4 py-3' : 'flex-1 flex-row items-center justify-center rounded-2xl border border-white/15 px-4 py-3'} onPress={() => void sortNearby()}><MaterialCommunityIcons name="crosshairs-gps" size={20} color="white" /><Text className="ml-2 font-black text-white">{language === 'es' ? 'Más cercanos' : 'Nearest'}</Text></Pressable></View></View>
      <FlatList
        ...
        renderItem={({ item }) => (
          <View className="overflow-hidden rounded-[30px] border border-white/10 bg-[#272522]">
            <View className="relative">
              ...
              <View className="absolute bottom-4 left-5 rounded-full bg-[#00b981] px-4 py-2"><Text className="font-black text-white">{tourismRegion(item)}</Text></View>
            ...
            <View className="p-6">
              ...
              <Text className="mt-2 text-base leading-6 text-[#c6c1bb]" numberOfLines={2}>{item.description || 'Información en proceso de verificación.'}</Text>
              <View className="mt-4 flex-row flex-wrap gap-2"><View className="flex-row items-center rounded-xl bg-[#233047] px-3 py-2"><MaterialCommunityIcons name="clock-outline" size={18} color="#a9c8ff" /><Text className="ml-2 font-black text-[#dce8ff]">~{travelTimeFromSanJose(item)}h {language === 'es' ? 'desde SJ' : 'from SJ'}</Text></View><View className="flex-row items-center rounded-xl bg-white/5 px-3 py-2"><MaterialCommunityIcons name="star" size={18} color="#ffac16" /><Text className="ml-2 font-black text-white">{item.average_rating ? item.average_rating.toFixed(1) : '—'} / 5</Text></View></View>
              <View className="mt-5 h-px bg-white/10" />
              <View className="mt-5 flex-row items-end justify-between">
                <View><Text className="text-xs font-bold uppercase tracking-wider text-[#9a958f]">{language === 'es' ? 'Tarifa Tico' : 'Foreigner price'}</Text><Text className="mt-1 text-xl font-black text-[#00e5a7]">{visitorType === 'tico' ? (item.price_national_crc == null ? 'Consultar' : item.price_national_crc === 0 ? 'Gratis' : formatPrice(item.price_national_crc)) : (item.price_foreigner_usd == null ? 'Check price' : item.price_foreigner_usd === 0 ? 'Free' : `$${item.price_foreigner_usd.toFixed(2)}`)}</Text></View>
                <View className="flex-row items-center gap-3">
                  <Pressable accessibilityLabel="Me gusta" className="flex-row items-center rounded-full px-3 py-3" onPress={() => void like(item)}><MaterialCommunityIcons name={item.liked ? 'heart' : 'heart-outline'} size={24} color={item.liked ? '#ff557d' : '#a8a29c'} /><Text className="ml-2 font-bold text-[#aaa49e]">{item.likes_count}</Text></Pressable>
                  <Pressable accessibilityRole="button" className="rounded-2xl bg-[#00b981] px-6 py-3" onPress={() => setSelected(item)}><Text className="font-black text-white">{language === 'es' ? 'Ver' : 'View'}</Text></Pressable>
                </View>
              </View>
            </View>
          </View>
  ...
  return (
    <Modal animationType="fade" onRequestClose={onClose} transparent visible>
      <View className="flex-1 items-center justify-center bg-black/75 p-3 md:p-8">
        <View className="max-h-full w-full max-w-5xl overflow-hidden rounded-[30px] bg-[#272522]">
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View className="gap-6 p-5 md:p-8">
              ...
              <View><Text className="text-lg font-black uppercase tracking-wider text-[#b8b3ad]">{language === 'es' ? 'Información para tu visita' : 'Visitor information'}</Text><Text className="mt-3 text-base leading-7 text-[#d4d0cb]">{place.description || (language === 'es' ? 'Información en proceso de verificación.' : 'Information is being verified.')}</Text></View>
              <View className="rounded-3xl border border-white/10 bg-[#302e2b] p-5">
                ...
                <View className="mt-5 flex-row flex-wrap gap-3 border-t border-white/10 pt-5">
                  <Pressable className="flex-row items-center rounded-2xl bg-[#13aee8] px-5 py-3" onPress={() => void openNavigation(place.latitude, place.longitude)}><MaterialCommunityIcons name="waze" size={21} color="white" /><Text className="ml-2 font-black text-white">{language === 'es' ? 'Ir con Waze' : 'Open in Waze'}</Text></Pressable>
                  {place.requires_sinac_booking && place.sinac_booking_url ? <Pressable className="flex-row items-center rounded-2xl bg-[#00b981] px-5 py-3" onPress={() => void Linking.openURL(place.sinac_booking_url!)}><MaterialCommunityIcons name="ticket-confirmation-outline" size={21} color="white" /><Text className="ml-2 font-black text-white">{language === 'es' ? 'Reservar en SINAC' : 'Book with SINAC'}</Text></Pressable> : null}
              ...
              {ferry ? <View className="rounded-3xl border border-[#00b981]/50 bg-[#16352d] p-5"><View className="flex-row items-center"><MaterialCommunityIcons name="ferry" size={30} color="#00e5a7" /><View className="ml-3"><Text className="text-lg font-black text-white">{language === 'es' ? 'Ferri recomendado' : 'Recommended ferry'}</Text><Text className="text-[#8cebcf]">{ferry.route} · {ferry.operator}</Text></View></View><Text className="mt-4 font-bold text-white">{language === 'es' ? 'Adulto' : 'Adult'} {formatPrice(ferry.adultFare)} · {language === 'es' ? 'Vehículo' : 'Vehicle'} {formatPrice(ferry.vehicleFare)} + {language === 'es' ? 'IVA' : 'VAT'}</Text><Text className="mt-2 leading-6 text-[#b7d8ce]">{language === 'es' ? 'Salidas' : 'Departures'}: {ferry.departures.join(' · ')}</Text><Pressable className="mt-4 self-start rounded-2xl bg-[#00b981] px-5 py-3" onPress={() => void Linking.openURL(ferry.ticketUrl)}><Text className="font-black text-white">{language === 'es' ? 'Ver horarios y comprar' : 'View schedule and buy'}</Text></Pressable></View> : null}
              <Pressable className="flex-row items-center justify-between rounded-3xl border border-white/10 bg-[#302e2b] p-5" onPress={() => setCommentsOpen((open) => !open)}><View className="flex-row items-center"><MaterialCommunityIcons name="comment-text-multiple-outline" size={24} color="#00e5a7" /><Text className="ml-3 text-lg font-black text-white">{commentsOpen ? (language === 'es' ? 'Ocultar comentarios' : 'Hide comments') : `${language === 'es' ? 'Ver comentarios' : 'View comments'} (${place.reviews_count})`}</Text></View><MaterialCommunityIcons name={commentsOpen ? 'chevron-up' : 'chevron-down'} size={25} color="#aaa49e" /></Pressable>
          ...
          <View className="flex-row items-center justify-between border-t border-white/10 bg-[#272522] p-4"><Pressable className="flex-row items-center rounded-2xl border border-white/15 px-5 py-3" onPress={() => void onLike(place)}><MaterialCommunityIcons name={place.liked ? 'heart' : 'heart-outline'} size={24} color={place.liked ? '#ff557d' : 'white'} /><Text className="ml-2 font-black text-white">{place.likes_count}</Text></Pressable><Pressable className="rounded-2xl bg-[#00b981] px-7 py-3" onPress={onClose}><Text className="font-black text-white">{language === 'es' ? 'Cerrar ficha' : 'Close'}</Text></Pressable></View>
        </View>
      </View>
    </Modal>
  );
...
function CommentsPanel({ comment, language, onComment, onPhoto, onRating, onSend, photo, rating, reviews, sending }: { comment: string; language: 'es' | 'en'; onComment: (value: string) => void; onPhoto: () => Promise<void>; onRating: (value: number) => void; onSend: () => Promise<void>; photo?: ImagePicker.ImagePickerAsset; rating: number; reviews?: Awaited<ReturnType<typeof getDestinationReviews>>; sending: boolean }) {
  return <View className="gap-4"><View className="rounded-3xl border border-white/10 bg-[#302e2b] p-5"><View className="flex-row gap-1">{[1,2,3,4,5].map((star) => <Pressable key={star} onPress={() => onRating(star)}><MaterialCommunityIcons name={star <= rating ? 'star' : 'star-outline'} size={27} color="#ffac16" /></Pressable>)}</View><TextInput className="mt-4 min-h-24 rounded-2xl bg-[#23211f] px-4 py-3 text-white" maxLength={800} multiline onChangeText={onComment} placeholder={language === 'es' ? 'Compartí tu experiencia…' : 'Share your experience…'} placeholderTextColor="#827d77" textAlignVertical="top" value={comment} />{photo ? <Image source={{ uri: photo.uri }} contentFit="cover" style={{ borderRadius: 16, height: 150, marginTop: 12, width: 200 }} /> : null}<View className="mt-4 flex-row justify-between"><Pressable className="flex-row items-center rounded-2xl border border-white/15 px-4 py-3" onPress={() => void onPhoto()}><MaterialCommunityIcons name="camera-plus-outline" size={21} color="#00e5a7" /><Text className="ml-2 font-black text-white">{language === 'es' ? 'Foto' : 'Photo'}</Text></Pressable><Pressable className="rounded-2xl bg-[#00b981] px-5 py-3" disabled={sending || (!comment.trim() && !photo)} onPress={() => void onSend()}><Text className="font-black text-white">{sending ? (language === 'es' ? 'Publicando…' : 'Posting…') : (language === 'es' ? 'Publicar' : 'Post')}</Text></Pressable></View></View>{reviews?.map((review) => <View className="rounded-3xl bg-[#302e2b] p-5" key={review.id}><View className="flex-row items-center">{review.avatar_url ? <Image source={{ uri: review.avatar_url }} contentFit="cover" style={{ borderRadius: 22, height: 44, width: 44 }} /> : <View className="h-11 w-11 items-center justify-center rounded-full bg-[#00b981]"><Text className="font-black text-white">{review.author_name.slice(0,1)}</Text></View>}<View className="ml-3 flex-1"><Text className="font-black text-white">{review.author_name}</Text><Text className="text-[#ffac16]">{'★'.repeat(review.rating)}</Text></View><Text className="text-xs text-[#8f8983]">{new Date(review.created_at).toLocaleDateString(language === 'es' ? 'es-CR' : 'en-US')}</Text></View>{review.comment ? <Text className="mt-4 text-base leading-6 text-[#d4d0cb]">{review.comment}</Text> : null}{review.photos?.[0] ? <Image source={{ uri: review.photos[0] }} contentFit="cover" style={{ borderRadius: 18, height: 220, marginTop: 14, width: '100%' }} /> : null}</View>)}</View>;
}

function Stat({ label, value }: { label: string; value: string }) {
  return <View className="flex-1 items-center border-r border-white/10 px-2 last:border-r-0"><Text className="text-center text-xs font-bold text-[#9d9892]">{label}</Text><Text className="mt-2 text-center text-lg font-black text-white">{value}</Text></View>;
}
