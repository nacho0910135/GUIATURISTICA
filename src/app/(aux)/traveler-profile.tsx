...
export default function TravelerProfileScreen() {
  ...
  const block = () => {
    ...
    Alert.alert(text('Bloquear usuario', 'Block user'), text('No verás su contenido y no podrá enviarte mensajes.', 'You will not see their content and they cannot message you.'), [{ text: text('Cancelar', 'Cancel') }, { text: text('Bloquear', 'Block'), style: 'destructive', onPress: () => void (async () => { await blockTraveler(id); router.back(); })() }]);
  };
  return <ScrollView className="flex-1 bg-ui-background dark:bg-ui-dark-background" contentContainerStyle={{ padding: 20, paddingBottom: 50 }}>
    ...
    {id !== viewerId ? <View className="mt-5 rounded-3xl bg-ui-surface p-5 dark:bg-ui-dark-surface"><Text className="text-lg font-black text-ui-text dark:text-ui-dark-text">{text('Mensaje privado', 'Private message')}</Text>{viewerId ? <><TextInput className="mt-3 rounded-2xl bg-ui-background p-4 text-ui-text dark:bg-ui-dark-background dark:text-ui-dark-text" multiline placeholder={text('Escribí un mensaje…', 'Write a message…')} placeholderTextColor="#8894aa" value={message} onChangeText={setMessage} /><Pressable disabled={busy || !message.trim()} className="mt-3 self-end rounded-full bg-ui-primary px-5 py-3 disabled:opacity-40" onPress={async () => { setBusy(true); try { await sendTravelerMessage(viewerId, id, message); setMessage(''); Alert.alert(text('Mensaje enviado', 'Message sent')); } finally { setBusy(false); } }}><Text className="font-black text-white">{text('Enviar', 'Send')}</Text></Pressable></> : <Pressable accessibilityRole="button" className="mt-3 flex-row items-center justify-center rounded-full bg-ui-primary px-5 py-3" onPress={() => requireAuth(text('enviar un mensaje directo', 'send a direct message'))}><MaterialCommunityIcons name="message-text-outline" size={19} color="white" /><Text className="ml-2 font-black text-white">{text('Enviar mensaje directo', 'Send direct message')}</Text></Pressable>}</View> : null}
