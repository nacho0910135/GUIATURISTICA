...
export function DestinationAIAssistant({ canUse, context, language, name, onSubscribe }: { canUse: boolean; context: string; language: 'es' | 'en'; name: string; onSubscribe: () => void }) {
  ...
  const ask = async (prompt: string, previous = messages) => {
    ...
    try {
      ...
      setError(language === 'es' ? 'No pudimos consultar Firebase AI Logic. Revisá la conexión e intentá de nuevo.' : 'We could not reach Firebase AI Logic. Check your connection and try again.');
  ...
  return <>
    <Pressable accessibilityRole="button" className="mt-3 min-h-11 flex-row items-center" onPress={launch}><MaterialCommunityIcons name="creation" size={19} color="#0B6B4F" /><Text className="ml-2 font-black text-ui-primary dark:text-ui-dark-primary">{language === 'es' ? 'Conocé más con IA' : 'Learn more with AI'}</Text>{!canUse ? <Text className="ml-2 rounded-full bg-ui-primary-soft px-2 py-1 text-[10px] font-black text-ui-primary">PRO</Text> : null}</Pressable>
    <Modal animationType="slide" onRequestClose={() => setOpen(false)} presentationStyle="fullScreen" visible={open}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1 bg-ui-background dark:bg-ui-dark-background" style={{ paddingTop: insets.top, paddingBottom: insets.bottom }}>
        ...
        <ScrollView className="flex-1" contentContainerStyle={{ padding: 20, gap: 14 }} keyboardShouldPersistTaps="handled">
          ...
          {error ? <Text accessibilityRole="alert" className="rounded-2xl bg-red-50 p-4 font-bold text-ui-danger dark:bg-red-950 dark:text-ui-dark-danger">{error}</Text> : null}
