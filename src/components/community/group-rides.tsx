  ...
  return <View className="mb-5">
    ...
    {rides.data?.map((ride) => <View className="mt-4 overflow-hidden rounded-card border border-ui-border bg-ui-surface dark:border-ui-dark-border dark:bg-ui-dark-surface" key={ride.id}>
      <View className="border-l-4 border-ui-secondary p-4">
        ...
        <View className="mt-4 gap-2"><View className="flex-row items-center"><MaterialCommunityIcons name="account-star-outline" size={20} color="#68737A" /><Text className="ml-2 flex-1 text-sm text-ui-text dark:text-ui-dark-text">{language === 'es' ? 'Organiza' : 'Organized by'}: <Text className="font-black">{organizerName(ride, language)}</Text></Text></View><Pressable accessibilityRole="link" className="min-h-11 flex-row items-center" onPress={() => void Linking.openURL(`https://www.google.com/maps/search/?api=1&query=${ride.latitude},${ride.longitude}`)}><MaterialCommunityIcons name="circle-outline" size={20} color="#0B6B4F" /><View className="ml-2 flex-1"><Text className="text-xs font-black uppercase text-ui-text-muted dark:text-ui-dark-text-muted">{language === 'es' ? 'Punto de reunión' : 'Meeting point'}</Text><Text className="font-black text-ui-text dark:text-ui-dark-text">{ride.place_name}</Text></View><MaterialCommunityIcons name="open-in-new" size={18} color="#0077A8" /></Pressable>{ride.destination_latitude != null && ride.destination_longitude != null ? <Pressable accessibilityRole="link" className="min-h-11 flex-row items-center" onPress={() => void Linking.openURL(`https://www.google.com/maps/search/?api=1&query=${ride.destination_latitude},${ride.destination_longitude}`)}><MaterialCommunityIcons name="map-marker" size={20} color="#1E5B75" /><View className="ml-2 flex-1"><Text className="text-xs font-black uppercase text-ui-text-muted dark:text-ui-dark-text-muted">{language === 'es' ? 'Destino' : 'Destination'}</Text><Text className="font-black text-ui-text dark:text-ui-dark-text">{ride.destination_name}</Text></View><MaterialCommunityIcons name="open-in-new" size={18} color="#0077A8" /></Pressable> : null}</View>
