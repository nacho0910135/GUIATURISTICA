...
export function GroupRides({ canWrite, language, topic, userId, requireAuth }: { canWrite: boolean; language: 'es' | 'en'; topic: TravelerTopic; userId?: string; requireAuth: (intent: string) => boolean }) {
  ...
  return <View className="mb-5">
    <View className="flex-row items-center justify-between">
      ...
      {canWrite ? <Pressable accessibilityRole="button" className="ml-3 min-h-11 flex-row items-center rounded-control bg-ui-primary px-3 dark:bg-ui-dark-primary" onPress={() => { if (requireAuth(language === 'es' ? 'programar una rodada' : 'schedule a ride')) { setCreating((value) => !value); setError(undefined); } }}><MaterialCommunityIcons name={creating ? 'close' : 'calendar-plus'} size={19} color="white" /><Text className="ml-1 font-black text-white">{creating ? (language === 'es' ? 'Cerrar' : 'Close') : (language === 'es' ? 'Programar' : 'Schedule')}</Text></Pressable> : null}
    </View>
    ...
    {rides.data?.map((ride) => <RideCard canWrite={canWrite} attendanceBusy={attendanceBusyId === ride.id} attendanceLocked={Boolean(attendanceBusyId)} key={ride.id} language={language} onAttendance={() => void toggleAttendance(ride)} onChanged={() => void rides.refetch()} requireAuth={requireAuth} ride={ride} userId={userId} />)}
  </View>;
}

function RideCard({ canWrite, attendanceBusy, attendanceLocked, language, onAttendance, onChanged, requireAuth, ride, userId }: { canWrite: boolean; attendanceBusy: boolean; attendanceLocked: boolean; language: 'es' | 'en'; onAttendance: () => void; onChanged: () => void; requireAuth: (intent: string) => boolean; ride: GroupRide; userId?: string }) {
  ...
  return <View className={`mt-4 overflow-hidden rounded-card border bg-ui-surface dark:bg-ui-dark-surface ${cancelled ? 'border-ui-danger dark:border-ui-dark-danger' : 'border-ui-border dark:border-ui-dark-border'}`}>
    ...
    <View className="p-4">
      ...
      <View className="mt-5 border-t border-ui-border pt-4 dark:border-ui-dark-border">
        <Pressable accessibilityRole="button" accessibilityState={{ expanded: commentsOpen }} className="min-h-11 flex-row items-center rounded-control bg-ui-muted px-3 active:opacity-75 dark:bg-ui-dark-muted" onPress={() => setCommentsOpen((open) => !open)}><MaterialCommunityIcons name="comment-text-multiple-outline" size={21} color="#0B6B4F" /><Text className="ml-2 flex-1 text-base font-black text-ui-text dark:text-ui-dark-text">{language === 'es' ? `Comentarios (${ride.comments.length})` : `Comments (${ride.comments.length})`}</Text><MaterialCommunityIcons name={commentsOpen ? 'chevron-up' : 'chevron-down'} size={22} color="#68737A" /></Pressable>
        {commentsOpen ? <View>
          ...
          {canWrite ? <TextInput accessibilityLabel={language === 'es' ? 'Comentario sobre la rodada' : 'Ride comment'} className="mt-3 min-h-20 rounded-control border border-ui-border bg-ui-muted px-4 py-3 text-ui-text dark:border-ui-dark-border dark:bg-ui-dark-muted dark:text-ui-dark-text" editable={!commentBusy} maxLength={1000} multiline onChangeText={setComment} placeholder={language === 'es' ? 'Escribí una duda o comentario…' : 'Write a question or comment…'} placeholderTextColor="#68737A" textAlignVertical="top" value={comment} /> : null}
          ...
          {canWrite ? <Pressable accessibilityRole="button" accessibilityState={{ busy: commentBusy, disabled: commentBusy || !comment.trim() }} className="mt-3 min-h-11 flex-row items-center justify-center rounded-control bg-ui-primary px-4 disabled:opacity-50 dark:bg-ui-dark-primary" disabled={commentBusy || !comment.trim()} onPress={() => void submitComment()}>{commentBusy ? <FrogLoader color="white" size="small" /> : <MaterialCommunityIcons name="send" size={18} color="white" />}<Text className="ml-2 font-black text-white">{language === 'es' ? 'Publicar comentario' : 'Post comment'}</Text></Pressable> : null}
        </View> : null}
      </View>
    </View>
  </View>;
