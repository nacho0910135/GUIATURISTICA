...
assert.match(screen, /Usar ubicación actual/);
assert.match(screen, /Ubicar en el mapa/);
...
assert.match(commerce, /request\.order\('id'\)\.limit\(50\)/);
assert.match(commerce, /getRoadRoutes\(origin/);
...
assert.match(screen, /directory\.data\?\.pages\.flatMap/);
...
assert.match(claimScreen, /Suscribirme y continuar/);
assert.match(claimGuardMigration, /service_already_claimed/);
assert.match(screen, /Registrar comercio/);
assert.match(screen, /hasActiveBusinessPlan/);
assert.match(screen, /pathname: "\/subscriptions"/);
...
assert.match(explore, /refreshUserLocation/);
assert.match(explore, /userLocation/);
assert.match(currentLocation, /Location\.Accuracy\.Highest/);
assert.match(currentLocation, /maximumAge: 0/);
assert.match(currentLocation, /isUsablePosition/);
