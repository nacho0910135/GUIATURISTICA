...
const offers = {
  universal_monthly: {
    plan: "no_ads",
    priceEnv: "STRIPE_PRICE_UNIVERSAL_MONTHLY",
    mode: "subscription",
  },
  universal_annual: {
    plan: "no_ads",
    priceEnv: "STRIPE_PRICE_UNIVERSAL_ANNUAL",
    mode: "subscription",
  },
  visitor_pass_30d: {
    plan: "no_ads",
    priceEnv: "STRIPE_PRICE_VISITOR_PASS_30D",
    mode: "payment",
  },
  business_monthly: {
    plan: "business",
  ...
  featured_monthly: {
    ...
    priceEnv: "STRIPE_PRICE_COMMERCE_FEATURED_MONTHLY",
    mode: "subscription",
  },
  banner_monthly: {
    campaignType: "banner",
    priceEnv: "STRIPE_PRICE_COMMERCE_BANNER_MONTHLY",
    mode: "subscription",
  },
} as const;
...
Deno.serve(async (request) => {
  ...
  if (isCampaign && offer.campaignType === "banner") {
    ...
    if (new Set((data ?? []).map((campaign) => campaign.service_id)).size >= 3)
  ...
  if (isCampaign) {
    ...
    if (offer.mode === "subscription") {
      ...
      form.set(
        "subscription_data[metadata][campaign_type]",
        offer.campaignType,
      );
      if (targetUrl)
        form.set("subscription_data[metadata][target_url]", targetUrl);
      if (imageUrl)
        form.set("subscription_data[metadata][image_url]", imageUrl);
    }
  } else {
