-- Keep trial access separate from paid-plan state so the UI can show the countdown.
...
as $$
  ...
  select jsonb_build_object(
    ...
    'hasPaidAccess', is_admin or has_personal_plan,
    ...
    'showTrialWarning', not is_admin and not has_personal_plan and trial_ends_at > now() and trial_ends_at <= now() + interval '5 days'
  ) from access;
$$;
