-- The moderation queue reads this column directly. RLS still decides which
-- rows each authenticated user may see.
grant select (moderation_status) on table public.fauna_species to authenticated;

-- Registration accepts the same public profile fields owners can edit later.
